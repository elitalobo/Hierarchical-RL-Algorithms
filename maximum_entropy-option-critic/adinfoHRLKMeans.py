...
def update_option(env, args, agent, replay_buffer_onpolicy, action_noise, update_num):
    ...
    while ite in range(int(update_num/4)) and flag==True:
        state, action, mean, log_std, reward, next_state, done, p_batch = replay_buffer_onpolicy.sample(
            args["option_minibatch_size"])
        state = tensor(state).to(device)
        action = tensor(action).to(device)
...
def train(args, agent):
    ...
    while total_step_cnt in range(args["total_step_num"]):
        ...
        for j in range(args["max_episode_len"]):
            ...
            replay_buffer.push(state, action.squeeze(0), mean.squeeze(0) , log_std.squeeze(0), reward,
                            state2, terminal, p)

            replay_buffer_onpolicy.push(state, action.squeeze(0), mean.squeeze(0), log_std.squeeze(0), reward,
                              state2, terminal, p)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--option-batch-size', help='batch size for updating option', default=2000)
    parser.add_argument('--option-update-num', help='iteration for updating option', default=2000)
