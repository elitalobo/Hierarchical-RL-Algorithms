...
class SoftActorCritic():
    ...
    def reset(self):
        ...
        self.separate_policies = args.separate_policies
        self.separate_q_values = args.separate_q_values
        self.sample_option_type = args.sample_option_type
        ...
        print("qlearn")
        print("separate policies " + str(self.separate_policies) + " sample_options "+ str(self.sample_option_type)+ " \n")
        ...
        self.soft_q_net1 = OptionsSoftQNetwork(self.state_dim, self.options_cnt, self.action_dim, self.hidden_dim).to(
                device)

        self.soft_q_net2 = OptionsSoftQNetwork(self.state_dim, self.options_cnt, self.action_dim, self.hidden_dim).to(
                device)

        self.beta_net = BetaBody(self.state_dim, self.options_cnt, self.hidden_dim).to(device)
        self.policy_net = OptionsPolicyNetwork(self.state_dim, self.options_cnt, self.action_dim, self.hidden_dim).to(device)
        if self.separate_policies:
            self.policy_net = DifferentOptionsPolicyNetwork(self.state_dim, self.options_cnt, self.action_dim, self.hidden_dim).to(device)
        ...
        self.sample_replay_buffer = ReplayBuffer(256)
    ...
    def get_actual_option(self,states,actions,epsilon=1e-6):
        ...
        if self.separate_policies >0:
            states = states.unsqueeze(2).repeat(1, 1, self.options_cnt).transpose(1, 2).reshape(
                self.batch_size * self.options_cnt, -1)
            # assert(torch.sum(self.mini_batch_tensor[0]!=self.mini_batch_tensor[1])>0)
            # assert(torch.sum(self.mini_batch_tensor[0]==self.mini_batch_tensor[4])>0)
            # assert(torch.sum(states[0]==states[1])>0)
            # assert(torch.sum(states[0]!=states[4])>0)
            new_action, new_log_prob, z, mean, log_std = self.policy_net.evaluate(states,self.mini_batch_tensor)

        else:

            new_action, new_log_prob, z, mean, log_std  = self.policy_net.evaluate(states)
    ...
    def get_action(self,state,option):
        ...
        assert(option.shape[-1]==1)
        option_idx = option.long().detach().flatten().cpu().numpy()
        if self.separate_policies == 0:
            action, log_prob, z, mean, log_std = self.policy_net.evaluate(state)


            return action[np.arange(len(option_idx)),option_idx,:].reshape(-1,self.action_dim), log_prob[np.arange(len(option_idx)),option_idx].reshape(-1,1), z, mean[np.arange(len(option_idx)),option_idx,:].reshape(-1,self.action_dim), log_std[np.arange(len(option_idx)),option_idx,:].reshape(-1,self.action_dim)

        #option = self.encode(option)
        action, log_prob, z, mean, log_std = self.policy_net.evaluate(state,option)
    ...
    def get_q_values(self,state,option,action):
        option_idx = option.long().flatten().detach().cpu().numpy()
        assert(option.shape[-1]==1)
        soft_q_value1 = self.soft_q_net1(state,action)[np.arange(len(option_idx)),option_idx].reshape(-1,1)
        soft_q_value2 = self.soft_q_net2(state,action)[np.arange(len(option_idx)),option_idx].reshape(-1,1)
    ...
    def update(self, batch_size, gamma=0.99, soft_tau=0.01, frame_idx=0, options_prob_episode=None, flag=False,
        ...
        option=None
        extended_state=None
        extended_next_state=None


        option = self.get_actual_option(state,action)

        extended_state = torch.cat((state, self.encode(option.detach())), -1)
        extended_next_state = torch.cat((next_state, self.encode(option.detach())), -1)

        reward = tensor(reward).unsqueeze(1).to(device)
        done = tensor(np.float32(done)).unsqueeze(1).to(device)

        predicted_q_value1, predicted_q_value2 = self.get_q_values(state,option,action)


        predicted_value = self.get_option_values(state,option).reshape(-1,1)
        predicted_next_value = self.get_option_values(next_state,option).reshape(-1,1)

        target_predicted_next_value = self.get_option_values(next_state,option,True).reshape(-1,1)
        ...
        #predicted_next_new_option_q_value = self.get_option_values(next_state,next_new_option)
        #target_predicted_next_new_option_q_value = self.get_option_values(next_state,next_new_option,True)
        ...
        advantage = (predicted_next_value - (
            predicted_next_new_option_q_value.reshape(-1, 1)))



        beta_loss = (beta_next[self.batch_iter, option_idx].reshape(-1, 1) * (
            advantage.detach())).mean() + self.bl * softmax_loss_vectorized(1-beta,option_idx).mean()


        alphas_option = tensor([self.alphas[option_idx[0].long()] for option_idx in option])
        predicted_q_value1, predicted_q_value2 = self.get_q_values(state,option, new_action)
        ...
        policy_loss = (
                              alphas_option * log_prob - predicted_new_q_value).mean()
        ...
        del state, option, action, reward, next_state, done,  extended_state, extended_next_state
        return policy_loss, beta_loss, value_loss, option_policy_loss, q_value_loss2, q_value_loss1
    ...
    def test(self, max_episode_len=1000):
        ...
        while episode_cnt < 10:
            ...
            for step in range(self.max_steps):
                ...
                actions, log_prob, z, mean, log_std = self.get_action(state.repeat(self.options_cnt, 1),
                                                                      self.options_tensor)
    ...
    def check_options(self):
        state, action, reward, next_state, done = self.sample_replay_buffer.sample(
            256,True)
        print(state[0])
        state = tensor(state)
        action = tensor(action)
        option = self.get_actual_option(state, action)
        option = option.reshape(-1)
        print(option)
        ...
        while frame_idx < self.max_frames:
            ...
            for step in range(self.max_steps):
                ...
                if frame_idx >=0:
                    ...
                    actions, log_prob, z, mean, log_std = self.get_action(state.repeat(self.options_cnt, 1), self.options_tensor)
                ...
                if len(self.replay_buffer) > self.batch_size:
                    if done:
                        print("train")
                        #print(options_used)
                        print(options_prob[0])
                        policy_loss, beta_loss, value_loss, option_alpha_loss, q_value_loss2, q_value_loss1 = self.update(self.batch_size, flag=done, frame_idx=frame_idx,
                                    options_prob_episode=options_probs[:, :step],
                                    soft_tau=self.soft_tau, actions_obs=None)

                        log_buffer.append(" policy_loss " + str(policy_loss) + " beta_loss " + str(beta_loss) + " value_loss " + str(value_loss) +
                        ...
                        print(" policy_loss " + str(policy_loss) + " beta_loss " + str(beta_loss) + " value_loss " + str(value_loss) +
                                          " option_alpha_loss " + str(option_alpha_loss) + " q_value_loss " + str(q_value_loss1) + " q_value_loss_2 " + str(q_value_loss2) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--value_lr', help='Discount factor', type=float, default=21e-4)
    parser.add_argument('--soft_q_lr', help='Discount factor', type=float, default=2e-4)
    parser.add_argument('--policy_lr', help='Discount factor', type=float, default=2e-4)  # 3e-5
