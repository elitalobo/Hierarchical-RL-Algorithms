...
class SoftActorCritic():
    ...
    def get_option_vals(self, states):
        ...
        for o in range(self.options_cnt):
            action_i, log_prob_i, _ , _, _ = self.get_action(states,o)

            Q_predict_i, _ = self.get_q_values(states,action_i)
    ...
    def get_actual_option(self,states,actions,epsilon=1e-6):
        ...
        if(self.c is None):
            ...
            options = torch.randint(0,self.options_cnt-1,(int(states.shape[0]),1))
            #print(options.shape)
            return options
        ...
        #dist = dist.reshape(-1,self.options_cnt)
    ...
    def update_option(self,num_updates,batch_size):
        for update_id in range(num_updates):
            ...
            len(self.replay_buffer_onpolicy))
        ...
        print("started")
        print(time.time())
        self.cl, self.c = Kmeans(state_action_pair, self.options_cnt,c=self.c)
        print(self.cl)
        print(time.time())
    ...
    def update_actor(self, batch_size, gamma=0.99, soft_tau=0.01, frame_idx=0, options_prob_episode=None, flag=False,
        ...
        return policy_loss, None, None, None, q_value_loss2, q_value_loss1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--value_lr', help='Discount factor', type=float, default=1e-3)
    parser.add_argument('--soft_q_lr', help='Discount factor', type=float, default=1e-3)
    parser.add_argument('--policy_lr', help='Discount factor', type=float, default=1e-3)  # 3e-5
    parser.add_argument('--beta_lr', help='Discount factor', type=float, default=1e-3)
    ...
    parser.add_argument('--update_policy_iter', help='num of episodes before evaluation', type=int, default=500)

    parser.add_argument('--update_option_iter', help='num of episodes before evaluation', type=int, default=4000)
