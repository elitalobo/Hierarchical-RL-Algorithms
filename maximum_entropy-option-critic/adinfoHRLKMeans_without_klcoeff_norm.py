...
def plot_tsne(X,y,label):
    ...
    plt.title('T-SNE plot for visualizing clusters in ' + label + " space")
...
def test(args,agent):
    ...
    #evaluate_deterministic_policy(agent, args, return_test, 0, env_test)
    states=[]
    options=[]
    all_actions=[]
    means=[]
    ...
    for test_iter in range(3):
        ...
        for j in range(args["max_episode_len"]):
            ...
            if j % args["temporal_num"] == 0 or not np.isscalar(option):
                #option, _, Q_predict = agent.softmax_option_target(state.unsqueeze(0))
                #option = option[0,0]
                option, q_max, q_predict = agent.max_option(state.unsqueeze(0))
                #print(option)
                option=option[0,0]
                #print(option)
                option = tensor(2.0).long()
                print(option)
                print(softmax(q_predict).detach())
                    #print(option)
                #print(option.shape)
            ...
            options.append(option.detach().cpu().numpy())
            all_actions.append(action.detach().cpu().numpy())
            means.append(mean.detach().cpu().numpy())
            log_stds.append(log_std.detach().cpu().numpy())
    ...
    all_actions = all_actions.reshape(all_actions.shape[0],all_actions.shape[-1])
    means = np.array(means)
    log_stds = np.array(log_stds)

    means = means.reshape(means.shape[0],all_actions.shape[-1])
    log_stds = log_stds.reshape(log_stds.shape[0],all_actions.shape[-1])


    options = np.array(options)
    plot_tsne(states,options,"state")
    plot_tsne(all_actions,options,"action")
    all_actions = tensor(all_actions)
    states= tensor(states)
    means = tensor(means)
    log_stds = tensor(log_stds)

    enc_output, option_out, output_option_noise, dec_output, option_input_concat = agent.option_net(states, all_actions.detach(), means.detach(), log_stds.detach())
    mean_log = torch.cat([means,log_stds],-1)
    mean_log_numpy = mean_log.detach().cpu().numpy()
    mean_log_numpy = mean_log_numpy.reshape(states.shape[0],-1)

    plot_tsne(mean_log_numpy,options,"state-action")
...
def main(args):
    for trial in range(args["trial_num"]):
        ...
        agent.load_weights(prefix="adInfoHRL/models/HopperBulletEnv-v0_2019-12-05-09-48-36")
        test(args,agent)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--overwrite-result', help='flag for overwriting the trial file', default=False)
    ...
    parser.set_defaults(change_seed=False)
