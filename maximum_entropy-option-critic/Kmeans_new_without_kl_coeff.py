...
def update_option(env, args, agent, replay_buffer_onpolicy, action_noise, update_num):
    for ite in range(update_num):
        ...
        #action, log_prob, z, mean, log_std = agent.predict_actor_option()
        noise_clip = 0.5
        noise_clip = 0.5
        noise = torch.clamp(Normal(0,action_noise).sample((next_option.shape[0] , agent.action_dim)) + action_noise,-noise_clip,noise_clip)
        next_action, next_log_prob = agent.predict_actor(next_state,next_option,target=True)
        ...
        target_q = torch.min(target_Q1, target_Q2) - agent.entropy_lr* next_log_prob
        ...
        for option_idx in range(args["option_ite"]):
            ...
            agent.train_option_autoencoder_kmeans(state,action,mean, log_std, y_i,predicted_v_i,p_batch)
    ...
    if agent.option_net.cluster_set==True:
        km = KMeans(n_clusters=agent.options_cnt, init=centers, max_iter=20000, n_init=1)
        flag=True
    else:
        km = KMeans(n_clusters=agent.options_cnt, init=centers, max_iter=10000, n_init=1)
    ...
    for nn in range(int(agent.test_num)):
        ...
        for t_test in range(int(agent.max_episode_len)):
            if t_test % int(args["temporal_num"]) == 0 or len(option_test) == 0:
                ...
                print(option_test)
                print(softmax(q_predict))
                #option_test = option_test[0]
                #print(q_predict/torch.sum(q_predict))
...
def train(args, agent):
    ...
    while total_step_cnt in range(args["total_step_num"]):
        ...
        for j in range(args["max_episode_len"]):
            ...
            replay_buffer_onpolicy.push(state.cpu().numpy(), action.squeeze(0), mean.squeeze(0), log_std.squeeze(0), reward,
                              state2.cpu().numpy(), terminal, p)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--entropy_coeff', help='cofficient for the mutual information term', default=0.0)
